      ...
      begin
        ...
        loop do
          page.search('a.summary').each do |link|
            ...
            # Venue params
            venue_page = agent.get(base_url + event_page.search('a.businessName').first.attributes['href'].value)
          ...
          if page.at('div#listingPagination').search('a').last.text.include?('Next')
            page = agent.get(base_url + page.at('div#listingPagination').search('a').last.attributes['href'].value)
          else
            break
          end
        end
